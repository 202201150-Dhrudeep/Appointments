...
export const MainToAll = () => {
  ...
  const convertTo24HourFormat = (timeStr) => {
      ...
      const timeSlotMapTomorrow = useMemo(() => {
        const map = timeSlots.reduce((acc, time) => {
          acc[time] = 1; // Mark all time slots as available initially
          return acc;
        }, {});
        ...
        return map;
      }, [appointmentsTomorrow, timeSlots]);
...
console.log(tomorrowResponse)
  ...
  const handleFormSubmit = async (e) => {
    ...
    if(timeSlotMap[formData.time]===0&&formData.date==="Today")
      {
        toast.error("Time has already been occupied")
        return;
      }

      if(timeSlotMapTomorrow[formData.time]===0&&formData.date==="Tomorrow")
        {
          toast.error("Time has already been occupied")
          return;
        }
